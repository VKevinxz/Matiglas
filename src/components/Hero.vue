...
<template>
  <section 
    ref="heroSection" 
    class="relative flex flex-col justify-center items-center min-h-screen overflow-hidden"
    :style="{
      '--mouse-x': `${mouseX}px`,
      '--mouse-y': `${mouseY}px`
    }"
  >
    ...
    <div class="absolute inset-0 bg-gradient-to-b from-black/30 via-black/40 to-background z-10"></div>
    
    <!-- Elementos decorativos animados -->
    <div class="absolute top-1/4 left-10 w-32 h-32 rounded-full bg-primary/20 blur-3xl animate-blob"></div>
    <div class="absolute bottom-1/4 right-10 w-40 h-40 rounded-full bg-secondary/20 blur-3xl animate-blob animation-delay-2000"></div>

    <!-- Contenido -->
    <div class="container mx-auto px-6 relative z-20">
      <div class="flex flex-col items-center gap-8 text-center">
        <div
          class="flex flex-col items-center transform transition-transform duration-700"
          :class="{ 'translate-y-0 opacity-100': isVisible, 'translate-y-10 opacity-0': !isVisible }"
        >
          <img
            src="/gamestation.png"
            alt="MATIGLAS"
            class="h-20 md:h-24 w-auto mb-4 animate-float"
          />
          <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold text-white mb-5 tracking-tight hero-title">
            MATIGLAS <span class="text-primary shadow-glow">E.I.R.L.</span>
          </h1>
          <p class="text-lg md:text-xl text-white/90 max-w-3xl mx-auto mb-2 leading-relaxed drop-shadow-md hero-subtitle">
            Soluciones integrales en <span class="text-primary font-semibold">{{ typingText }}</span><span class="typing-cursor">|</span>
          </p>
          <p class="text-white/80 max-w-2xl mx-auto leading-relaxed drop-shadow-md">
        ...
        <div 
          class="flex flex-col sm:flex-row gap-4 mt-6 transform transition-all duration-700 delay-200 w-full max-w-md mx-auto"
          :class="{ 'translate-y-0 opacity-100': isVisible, 'translate-y-10 opacity-0': !isVisible }"
        >
          <Button variant="default" size="lg" class="group btn-hover-effect w-full sm:w-auto relative overflow-hidden">
  ...
  <section
    id="inicio"
    class="container py-24 sm:py-32 min-h-[92vh] md:h-[92vh] flex flex-col justify-center overflow-hidden"
  >
    <div class="grid lg:grid-cols-2 place-items-center gap-8 lg:pt-8">
      <AnimateOnScroll>
        <div class="flex flex-col items-start space-y-4 z-10">
          <div class="inline-block rounded-lg bg-muted px-3 py-1 text-lg cta-badge relative">
            <span class="text-primary">MATIGLAS E.I.R.L.</span>
          ...
          <div class="flex flex-col sm:flex-row gap-4 pt-4">
            <Button variant="default" class="hero-btn-primary">
...
.shadow-glow {
  text-shadow: 0 0 10px rgba(241, 229, 20, 0.5), 0 4px 8px rgba(0, 0, 0, 0.5);
}
...
.hover-effect {
  ...
  inset: 0;
  border-radius: 1rem;
  background: linear-gradient(45deg, rgba(241, 229, 20, 0.2), rgba(25, 81, 155, 0.2));
  opacity: 0;
...
.circle-2 {
  width: 80px;
  height: 80px;
  background-color: rgba(25, 81, 155, 0.15);
  bottom: 20px;
  left: -20px;
}
...
.hero-btn-primary:hover {
  ...
  box-shadow: 0 7px 14px rgba(241, 229, 20, 0.3);
}
